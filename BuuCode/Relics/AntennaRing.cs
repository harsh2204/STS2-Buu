...
public sealed class AntennaRing() : BuuRelic
{
    private const decimal KiAtRoundOne = 1m;
    private const int RoundWithEffect = 1;

    public override RelicRarity Rarity => RelicRarity.Common;

    public override async Task AfterSideTurnStart(CombatSide side, CombatState combatState)
    {
        if (side != Owner.Creature.Side || combatState.RoundNumber != RoundWithEffect)
            return;
